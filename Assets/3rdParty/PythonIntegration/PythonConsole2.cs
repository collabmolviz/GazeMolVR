...
public class PythonConsole2 : MonoBehaviour {
    ...
    bool ShouldToggleConsole() {
        return Input.GetKey(KeyCode.LeftControl) && Input.GetKeyDown(KeyCode.Space);
    ...
    void RecreateScope() {
        ...
        // PythonUtils.GetEngine().Runtime.IO.SetOutput(ms, Encoding.Default);
    ...
    public IEnumerator ExecuteScript(string path) {
        ...
        foreach (string c in scriptCommands) {
            ...
            else if (ismulti == false && multilineBlock == false) {
                if (ctrim.Length > 1)
                    commandBlocks.Add(c);
                continue;
            }
