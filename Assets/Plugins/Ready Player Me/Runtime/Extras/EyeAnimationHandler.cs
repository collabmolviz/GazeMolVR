...
namespace ReadyPlayerMe
{
    ...
    public class EyeAnimationHandler : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (headMesh == null)
            {
                //Debug.LogError("No SkinnedMeshRenderer found in GameObject");
                return;
            }

            // If headMesh.sharedMesh is null, then the SkinnedMeshRenderer doesn't have a mesh
            if (headMesh.sharedMesh == null)
            {
                //Debug.LogError("No mesh found in SkinnedMeshRenderer");
                return;
            }
