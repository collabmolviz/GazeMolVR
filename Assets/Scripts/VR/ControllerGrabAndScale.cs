...
namespace UMol
{
    ...
    public class ControllerGrabAndScale : MonoBehaviour
    {
        ...
        void searchOtherController()
        {
            ...
            {
                otherController = GameObject
                    .Find("RightHand")
                    .GetComponent<ControllerGrabAndScale>();
            }
            else
            {
                otherController = GameObject
        ...
        private void triggerClicked()
        {
            ...
            if (a == null)
            {
                a = raycaster.customRaycastAtomBurst(
                    cpose.pos,
                    vecDir30,
                    ref p,
                    ref isExtrAtom,
            ...
            if (a != null)
            {
                Transform atomPar = sm.structureToGameObject[
                    a.residue.chain.model.structure.name
                ].transform;
            ...
            {
                if (
                    otherController.isClicking
                    && //Both controllers are clicking
                    !grabbedUI
                    &&
                    //None grabs anything
                    (
        ...
        private void grabGroupMolecule(Transform toGrab)
        {
            ...
            if (structure != null)
            {
                HashSet<UnityMolStructure> strucOfGroup = API.APIPython.getStructuresOfGroup(
                    structure.groupID
                );
        ...
        public IEnumerator bringMenuCloser()
        {
            ...
            if (mainUIGo != null && head != null)
            {
                ...
                for (int i = 1; i < steps / 4; i++)
                {
                    ...
                    mainUIGo.transform.position = Vector3.Lerp(
                        mainUIGo.transform.position,
                        targetPos,
                        tt
                    );

                    Vector3 newRot = new Vector3(
                        Mathf.LerpAngle(mainUIGo.transform.eulerAngles.x, targetRot.x, tt),
        ...
        void Update()
        {
            if (isScaling && !isGroupGrabbed)
            {
                ...
                float magNow =
                    sizeScale * (transform.position - otherController.transform.position).magnitude;
                float diff = magNow - startMagnitude;
                float scaleVal = Mathf.Max(0.005f, Mathf.Min(0.25f, scaleStart.x + diff));
