...
public class UIManager : MonoBehaviourPunCallbacks {	
	...
	void Awake() {
		...
		if (tourUITransform == null) {
			tourUITransform = GameObject.Find(mainUIName + "/Selection Scroll View/Viewport/Content/TourSelections").transform;
		}
	...
	public IEnumerator fetch(string t) {
		...
		try {
			...
			{				
				photonView.RPC("LoadMolecule", RpcTarget.All, t, mmCIF, readHTM, bioAssembly);
			}
			*/			
		}
	...
	/*
	[PunRPC]
    void LoadMolecule(string t, bool mmCIF, bool readHTM, bool bioAssembly)
    {        
        //APIPython.fetch(t);  
		APIPython.fetch(t, mmCIF, readHTM, bioAssembly: bioAssembly);      
    }
	*/
	...
	private void setupRepresentationButtons(GameObject newButtonGo, UnityMolSelection sel, string repT = "") {
		...
		case "surface":
			...
			delegate {
				if (APIPython.isSurfaceAOOn(selName, "s")) {
					APIPython.clearSurfaceAO(selName, "s");
				}
				else {
					APIPython.computeSurfaceAO(selName, "s");
				}
			});
