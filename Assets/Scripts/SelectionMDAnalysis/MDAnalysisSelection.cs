...
public class RangeResnumSelection : MDASelection {
    ...
    public RangeResnumSelection(MDAnalysisSelection mda, string t, int p) : base(mda, t, p) {
        uppers = new List<int>();
        lowers = new List<int>();
    }
    public override void init(List<string> tokens, ref int curId) {
    ...
    public RangeResnumSelection (RangeResnumSelection cpy) : base(cpy) {
        this.valueOffset = cpy.valueOffset;
        this.uppers = cpy.uppers;
        this.lowers = cpy.lowers;
    }
    public override MetaClass Clone() { return new RangeResnumSelection(this);}
...
public class AroundSelection : DistanceSelection {
    ...
    public override HashSet<UnityMolAtom> apply(HashSet<UnityMolAtom> group) {
        ...
        if (sameS) { //Atoms are from the same structure -> we can use the structure spatial search
        ...
        // result.Clear();
        float startkd = Time.realtimeSinceStartup;
        ...
        HashSet<int> reshs = new HashSet<int>();
        for (int i = 0; i < rangeResults.Length; i++) {
            for (int j = 0; j < rangeResults[i].Length; j++) {
                reshs.Add(rangeResults[i][j]);
            }
        }
        ...
        foreach (int id in reshs) {
            result.Add(groupAtoms[id]);
        }
        allPoints.Dispose();
        qPoints.Dispose();

        // Debug.Log("Time for within KDTREE: " + (1000.0f * (Time.realtimeSinceStartup - startkd)).ToString("f3") + " ms");
...
public class InSphereSelection : DistanceSelection {
    ...
    public override HashSet<UnityMolAtom> apply(HashSet<UnityMolAtom> group) {
        ...
        var allPoints = new NativeArray<float3>(group.Count, Allocator.Persistent);
        var qPoints = new NativeArray<float3>(1, Allocator.Persistent);
        ...
        // And just run immediately now. This will run on multiple threads!
        batchRange.ScheduleBatch(qPoints.Length, Mathf.Max(1, qPoints.Length / 32)).Complete();
...
public class InRectangleSelection : DistanceSelection {
    ...
    public InRectangleSelection(MDAnalysisSelection mda, string t, int p) : base(mda, t, p) {
    }

    public override void init(List<string> tokens, ref int curId) {
        lowerLeft.x = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        lowerLeft.y = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        lowerLeft.z = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);

        xaxis.x = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        xaxis.y = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        xaxis.z = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);

        yaxis.x = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        yaxis.y = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        yaxis.z = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);

        zaxis.x = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        zaxis.y = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
        zaxis.z = float.Parse(tokens[curId++], System.Globalization.CultureInfo.InvariantCulture);
    }
    public override HashSet<UnityMolAtom> apply(HashSet<UnityMolAtom> group) {
        HashSet<UnityMolAtom> result = new HashSet<UnityMolAtom>();
    ...
    public InRectangleSelection (InRectangleSelection cpy) : base(cpy) {
        // this.sel = cpy.sel.Clone();
    ...
    public override MetaClass Clone() { return new InRectangleSelection(this);}
}
...
public class WaterSelection : MDASelection {
    ...
    public static HashSet<string> waterResidues = new HashSet<string> {
        "HOH", "WAT", "SOL", "TIP3", "TP3M", "SPC", "H2O", "TIP", "W"
    };
...
public class IonsSelection : MDASelection {
    ...
    public static  HashSet<string> ionNames = new HashSet<string> {
        ...
        "MG", "MN", "MO", "NI", "PT", "RH", "RU", "SE", "TA", "ZN", "NA"
