...
public class ReticlePoser_US02 : MonoBehaviour
{
    ...
    public GameObject controllerRayHitsProteinGO;
    ...
    protected virtual void LateUpdate()
    {
        ...
        {
            targetReticle.transform.GetChild(0).GetComponent<MeshRenderer>().enabled = true;
            controllerRayHitsProteinGO.SetActive(true);
        }

        // don't show sphere reticle if ray hits "protein" gameobject, but accessing its position
        if (ShouldDisableTargetReticle(result))
        {
            if (targetReticle != null)
            {
                //targetReticle.gameObject.SetActive(false);
                targetReticle.transform.GetChild(0).GetComponent<MeshRenderer>().enabled = false;
                controllerRayHitsProteinGO.SetActive(false);
            }
        }
    ...
    private bool ShouldDisableTargetReticle(UnityEngine.EventSystems.RaycastResult result)
    {
        if (result.isValid && result.gameObject.CompareTag("protein"))
        {
            return true;
        }
        return false;
